...
IEnumerator Settings::SearchForDevices()
{
    while (true)
    {
...
void Settings::LayoutDeviceButtons(List<YawDevice> devices)
{
    foreach (GameObject deviceButton in deviceButtons)
    {
        Destroy(deviceButton);
...
void Settings::UDPPortInputFieldTextDidChange(InputField inputField)
{
    availableDevices.Clear();
...
void Settings::TCPPortInputFieldTextDidChange(InputField inputField)
{
    int portNumber;
    if (int.TryParse(inputField.text, out portNumber))
    {
...
void Settings::IPAddressInputFieldTextDidChange(InputField inputField)
{
...
void Settings::SetDeviceFromPortAndIp()
{
...
void Settings::ConnectButtonPressed()
{
...
void Settings::DisconnectButtonPressed()
{
...
void Settings::DeviceListItemPressed(YawDevice device)
{
...
void Settings::SourceOfMotionDropDownValueChanged(Dropdown dropdown)
{
...
void Settings::RotationMultiplierInputFieldTextDidChange(InputField inputField)
{
    float yaw;
    float pitch;
    float roll;
...
void Settings::AccelerationMultiplierInputFieldTextDidChange(InputField inputField)
{
    float pitch;
    float roll;
...
void Settings::LateralForceMultiplierInputFieldTextDidChange(InputField inputField)
{
    float roll;
...
void Settings::RotationLimitInputFieldTextDidChange(InputField inputField)
{
...
void Settings::DidFoundDevice(YawDevice device)
{
    bool contains = false;
...
bool Settings::SameDevice(YawDevice device, YawDevice toDevice)
{
    if (device.Id == toDevice.Id && device.TCPPort == toDevice.TCPPort && device.UDPPort == toDevice.UDPPort) return true;
    return false;
}

void Settings::YawLimitDidChange(int currentLimit)
{
    yawLimitInputField.text = currentLimit.ToString();
}

void Settings::TiltLimitsDidChange(int pitchFrontLimit, int pitchBackLimit, int rollLimit)
{
...
void Settings::RememberDeviceToggleValueDidChange(Toggle toggle)
{
    YawController.Instance().SetRememberDevice(toggle.isOn);
}

void Settings::DidDisconnectFrom(YawDevice device)
{
    ShowError("Device disconnected");
}

void Settings::ControllerStateChanged(ControllerState state)
{
    RefreshLayout(state);
}

void Settings::RefreshLayout(ControllerState state) {
    switch (state)
    {
...
void Settings::ShowError(string error, int duration = 10)
{
    if (errorText.text != "")
    {
...
IEnumerator Settings::ClearError(int duration)
{
